...
* @since 0.9.8
*/
class RBackendGenerator extends RGenerator
{
		...
		try
		{
			...
			foreach( $this->_items as $type=>$items )
			{
				...
				foreach( $items as $name )
				{
					...
					{
						// Insert item
						$sql = "INSERT INTO {$itemTable} (name, type,description, data)
							VALUES (:name, :type, :description,:data)";
						$command = $this->db->createCommand($sql);
						$command->bindValue(':name', $name);
						$command->bindValue(':type', $type);
                                                $command->bindValue(':description',$name.Rights::t('core', 'BackendControllers'));
	...
	*/
	protected function getControllersInModules($path)
	{
		...
		if( file_exists($modulePath)===true )
		{
			...
			{
				if( substr($entry, 0, 1)!=='.' && $entry!=='rights' && $entry!=='gii' )
				{
					$subModulePath = $modulePath.DIRECTORY_SEPARATOR.$entry;
					if( file_exists($subModulePath)===true )
					{
						$items[ $entry ]['controllers'] = $this->getControllersInPath($subModulePath.DIRECTORY_SEPARATOR.'admin');
